...
def main():
    ...
    for idx, row in matrix.iterrows():
        ...
        cols_for_transform = [c for c in X_train_drop.columns if c != ["Sex_num", "ICI_num",
                                                                       "cancer_num", "transf HPV16/18 copies per ml of plasma D1",
                                                                       "HPV16/18 copies per ml of plasma D1"]]

        ## Transformation??
        X_train_with_logs = log_transforms(X_train_drop, X_train_drop.columns)
        #print(X_train_with_logs.filter(like="_log").head())

        X_test_with_logs = log_transforms(X_test_drop, X_test_drop.columns)
        ...
        out_dir = Path(args.out_root) / "metrics" / stamp  / f"{dataset}__{featureset}"
